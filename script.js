...
const MOCK_VEHICLES = {
  ...
  jet: [
    { lat: -10.9480, lng: -37.0740, label: "Patinete #J-14", id: "jet-01", bateria: 92, status: "disponível" },
    { lat: -10.9455, lng: -37.0762, label: "Patinete #J-07", id: "jet-02", bateria: 67, status: "disponível" },
    { lat: -10.9502, lng: -37.0718, label: "Patinete #J-23", id: "jet-03", bateria: 41, status: "disponível" },
    { lat: -10.9435, lng: -37.0795, label: "Patinete #J-31", id: "jet-04", bateria: 85, status: "disponível" },
    { lat: -10.9468, lng: -37.0728, label: "Patinete #J-09", id: "jet-05", bateria: 18, status: "bateria baixa" },
    { lat: -10.9512, lng: -37.0755, label: "Patinete #J-42", id: "jet-06", bateria: 74, status: "disponível" }
  ],
  onibus: [
...
function abrirMapa(modalSelecionado) {
  const modaisValidos = ["uber", "onibus", "jet"];
  const modalMapa = modaisValidos.includes(modalSelecionado) ? modalSelecionado : "uber";
...
function renderVehicleMarkersLeaflet(modal) {
  ...
  const color = modal === "uber" ? "#ff6b35" : modal === "jet" ? "#a855f7" : "#0984e3";
  const iconName = modal === "uber" ? "car" : modal === "jet" ? "zap" : "bus";

  veiculos.forEach((veiculo) => {
    const extraClass = modal === "jet" ? batteriaClass(veiculo.bateria) : (veiculo.lotacaoNivel || "");
    L.marker([veiculo.lat, veiculo.lng], {
      icon: L.divIcon({
        className: "",
        html: `<div class="vehicle-pin ${modal} ${extraClass}"><i data-lucide="${iconName}" aria-hidden="true"></i></div>`,
        iconSize: [36, 36],
        iconAnchor: [18, 18]
      }),
      title: veiculo.label
...
function trocarModal(modal) {
  ...
  renderPainelJet(modal);
  state.modalRecomendado = modal;
}

function atualizarCardProximo(modal) {
  if (modal === "jet") {
    const disponiveis = MOCK_VEHICLES.jet.filter(v => v.status === "disponível");
    const proximo = disponiveis[0] || MOCK_VEHICLES.jet[0];
    document.querySelector("#card-proximo .card-icon").textContent = "🛴";
    document.getElementById("label-proximo").textContent = proximo.label;
    document.getElementById("eta-proximo").textContent = `Bateria: ${proximo.bateria}% | Status: ${proximo.status}`;
    return;
  }
  const veiculos = MOCK_VEHICLES[modal] || MOCK_VEHICLES.uber;
...
function popupVeiculo(modal, veiculo) {
  if (modal === "jet") {
    const bateriaColor = veiculo.bateria >= 60 ? "#00b894" : veiculo.bateria >= 30 ? "#f5c842" : "#d94841";
    return `<strong>${veiculo.label}</strong><br>Bateria: <b style="color:${bateriaColor}">${veiculo.bateria}%</b><br>Status: <b>${veiculo.status}</b><br><small>Patinete elétrico Jet</small>`;
  }
...
function renderPainelJet(modal) {
  const painel = document.getElementById("painel-jet");
  if (!painel) return;
  const mostrar = modal === "jet";
  painel.classList.toggle("hidden", !mostrar);
  if (!mostrar) return;

  const lista = document.getElementById("lista-jet");
  const ordenados = [...MOCK_VEHICLES.jet].sort((a, b) => b.bateria - a.bateria);

  lista.innerHTML = ordenados.map((v) => {
    const nivel = batteriaClass(v.bateria);
    const labelNivel = v.bateria >= 60 ? "Pronto" : v.bateria >= 30 ? "Moderado" : "Bateria baixa";
    return `
      <article class="occupancy-item">
        <div class="occupancy-head">
          <strong>${v.label}</strong>
          <span class="occupancy-badge ${nivel === "cheio" ? "tranquilo" : nivel === "medio" ? "moderado" : "lotado"}">${labelNivel}</span>
        </div>
        <div class="occupancy-bar" aria-label="Bateria ${v.bateria}%">
          <div class="occupancy-fill ${nivel === "cheio" ? "" : nivel === "medio" ? "moderado" : "lotado"}" style="width:${v.bateria}%; background:${nivel === "cheio" ? "#00b894" : nivel === "medio" ? "#f5c842" : "#d94841"}"></div>
        </div>
        <span class="occupancy-meta">🔋 ${v.bateria}% bateria | Status: ${v.status}</span>
      </article>
    `;
  }).join("");
}

function batteriaClass(bateria) {
  if (bateria >= 60) return "cheio";
  if (bateria >= 30) return "medio";
  return "baixo";
}
